...
# Previous Script: Individual dBBMMs

# Here we will extract the 95% and 50% contours for the utilization distributions. We will calculate their areas and map them
# Overall Steps:
  ...
  # 6. Plot with ggplot
  # 7. Individual maps are combined with outside software for publication; the first map for Turtle A includes a legened used for all maps (not included in code for other maps)
...
st_area(UD.As.95) # 127949667 [m^2]
# Satellite 50% 
...
st_area(UD.Ass.95) # 127949667 [m^2]
# Satellite Subset 50% 
...
plot(UD.Ass.50)
st_area(UD.Ass.50) # 10440967 [m^2]
...
st_area(UD.Aas.50) # 581397 [m^2]

# Matching Temporal Duration UD plots
A_equal <- ggplot() +
  geom_sf(data = land, fill = "grey", col = "grey") +
  geom_sf(data = UD.As.95, aes(fill = "deepskyblue1"), col = "deepskyblue1", lwd = 0.25, alpha = 0.3, show.legend = "polygon") +
  geom_sf(data = UD.As.50, aes(fill = "blue2"), col = "blue2", lwd = 0.25, alpha = 0.5, show.legend = "polygon") +
  geom_sf(data = UD.Aa.95, aes(fill = "violet"), col = "violet", lwd = 0.25, alpha = 0.7, show.legend = "polygon") +
  geom_sf(data = UD.Aa.50, aes(fill = "violetred"), col = "violetred", lwd = 0.25, alpha = 0.5, show.legend = "polygon") +
...
st_area(UD.Das.50) # 417574.1

# Macthing Temporal Duration UD plot
D_equal <- ggplot() +
  geom_sf(data = land, fill = "grey", col = "grey") +
